// index.js — API con: recent-used + heartbeat + grace anti-colisiones
...
// Evitar reusar el mismo server por X min
...
// 🔸 NUEVO: mantener locks vivos y gracia post-release
...
function sweepPlace(placeId){
  ...
  for (const [jid,rec] of p.items){
    ...
    if (rec.lock && rec.lock.until <= t){ rec.lock=null; rec.reassignAfter = t + MIN_REASSIGN_DELAY_SEC*1000; }
...
async function handleNext(req,res,u){
  ...
  send(res,200,{ ok:true, jobId:rec.jobId, placeId, lock:peek?null:rec.lock, players:rec.players, maxPlayers:rec.maxPlayers, region:rec.region, ping:rec.ping });
}
...
async function handleConfirm(req,res,u){
  ...
  if(!placeId||!jobId) return send(res,400,{ok:false,error:"placeId and jobId are required"});
  const p=getPlace(placeId); sweepPlace(placeId);
  if (p.items.has(jobId)){ p.items.delete(jobId); markRecentUsed(p,jobId); p.metrics.totalConfirms++; return send(res,200,{ok:true,removed:true}); }
  markRecentUsed(p,jobId); p.metrics.totalConfirms++; return send(res,200,{ok:true,removed:false,reason:"not_found"});
}
...
async function handleRelease(req,res,u){
  if(!checkKey(req,u)) return unauthorized(res);
  const b=await parseBody(req); const placeId=String(b.placeId||""), jobId=String(b.jobId||""); const reason=String(b.reason||"");
  if(!placeId||!jobId) return send(res,400,{ok:false,error:"placeId and jobId are required"});
  const p=getPlace(placeId); sweepPlace(placeId);
  const rec=p.items.get(jobId);
  if(!rec){ markRecentUsed(p,jobId); return send(res,200,{ok:true,released:false,reason:"not_found"}); }
  rec.lock=null;
...
// 🔸 NUEVO: heartbeat para extender lock mientras el bot teleporta
...
async function handleStats(req,res,u){
  ...
  if(placeId){ sweepPlace(placeId); const p=getPlace(placeId);
    return send(res,200,{ ok:true, placeId, pool:p.items.size, recentUsed:p.recentUsed.size,
      metrics:p.metrics, config:{ MAX_PER_PLACE,LOCK_LEASE_SEC,IGNORE_IF_PLAYERS_GE,MIN_FREE_SLOTS_REQ,MAX_AGE_MIN,PREFER_NEWER_BONUS_SEC,RECENT_USED_TTL_MIN,RECENT_USED_MIN_POOL,LOCK_HEARTBEAT_EXTEND_SEC,HEARTBEAT_MAX_SEC,MIN_REASSIGN_DELAY_SEC } });
  } else {
...
async function handleAll(req,res,u){
  ...
  for (const rec of p.items.values()){
    rows.push({ jobId:rec.jobId, players:rec.players, maxPlayers:rec.maxPlayers, hasLock:!!rec.lock, lockBy:rec.lock?.by||null, lockUntil:rec.lock?.until||null, coolUntil:rec.coolUntil||null, reassignAfter:rec.reassignAfter||null, badCount:rec.badCount||0, ts:rec.ts, lastSeen:rec.lastSeen });
  }
...
const server=http.createServer(async (req,res)=>{
  ...
  if (m==="OPTIONS"){ res.writeHead(204,{ "Access-Control-Allow-Origin":"*","Access-Control-Allow-Headers":"Content-Type,x-api-key","Access-Control-Allow-Methods":"GET,POST,OPTIONS" }); return res.end(); }
  try{
  ...
  }catch(e){ return send(res,500,{ok:false,error:"Server error",details:String(e&&e.message||e)}); }
});
